...
def test_agent_completes_successfully():
    agent = GeoScoutAgent(planner=MockGeoPlanner())
...
def test_agent_selects_hotspot_tool():
    ...
    assert state.tool_calls[0].tool_name == "detect_change_hotspots"
...
def test_agent_uses_expected_threshold():
    ...
    assert state.tool_calls[0].arguments["threshold"] == -0.10
