...
def get_summaries(data):
    return {summary["model"]: summary for summary in data["summaries"]}
...
def save_performance_figure(summaries):
    ...
    grounded = [summaries[model]["grounded_correctness"] * 100 for model in models]
    answer = [summaries[model]["answer_correctness"] * 100 for model in models]
    evidence = [summaries[model]["evidence_support_rate"] * 100 for model in models]
...
def save_latency_figure(summaries):
    ...
    latency = [summaries[model]["mean_latency_ms"] / 1000 for model in models]
...
def save_tool_usage_figure(summaries):
    ...
    tool_calls = [summaries[model]["mean_tool_calls"] for model in models]

    llm_calls = [summaries[model]["mean_llm_calls"] for model in models]
...
def save_failure_figure(data):
    ...
    for model in models:
        ...
        grounded_failures.append(sum(not result["grounded_correct"] for result in results))

        evidence_failures.append(sum(not result["evidence_supported"] for result in results))

        answer_failures.append(sum(not result["answer_correct"] for result in results))
